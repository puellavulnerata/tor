...
channel_tls_free_all(void)
{
  ...
  if (channel_tls_listener) {
     ...
     */
    l = channel_tls_listener;
    log_debug(LD_CHANNEL,
              "Closing channel_tls_listener with ID " U64_FORMAT
              " at %p.",
              U64_PRINTF_ARG(l->global_identifier),
              l);
    channel_listener_unregister(l);
    channel_listener_mark_for_close(l);
    channel_listener_free(l);

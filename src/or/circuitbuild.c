...
get_unique_circ_id_by_chan(channel_t *chan)
{
  ...
  high_bit = (chan->circ_id_type == CIRC_ID_TYPE_HIGHER) ? 1<<15 : 0;
  do {
...
circuit_extend(cell_t *cell, circuit_t *circ)
{
  ...
  if (tor_memeq(id_digest, TO_OR_CIRCUIT(circ)->p_chan->identity_digest,
              DIGEST_LEN)) {
    log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
  ...
  if (circ->cpath->state == CPATH_STATE_AWAITING_KEYS) {
    ...
    if (!circ->has_opened && !circ->build_state->onehop_tunnel) {
      ...
      guard = entry_guard_get_by_id_digest(
              circ->_base.n_chan->identity_digest);
      if (guard) {
                 ...
                 const char *keys)
{
  ...
  if (!channel_is_local(circ->p_chan) &&
      !channel_nonopen_was_started_here(circ->p_chan)) {
